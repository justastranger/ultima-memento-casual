...
namespace Server.Mobiles
{
    public class TrainingMagery : Citizens
    {
            ...
            if (DateTime.Now >= m_NextTalk)
            {
                foreach (Item pentagram in this.GetItemsInRange(4))
                {
                    if (pentagram is MagicHit)
                    {
                        ...
                        m_NextTalk = (DateTime.Now + TimeSpan.FromSeconds(Utility.RandomMinMax(10, 15)));
                    }
                }
            }
        }
        ...
        public override void OnAfterSpawn()
        {
            ...
            {
                Item wand = new JewelryRing();
                wand.Layer = Layer.OneHanded;
                wand.Name = "magic wand";
                wand.ItemID = Utility.RandomList(0xDF2, 0xDF3, 0xDF4, 0xDF5);
                ResourceMods.SetRandomResource(false, false, wand, CraftResource.None, true, this);
                wand.Movable = false;
                AddItem(wand);
            }
        }
