...
namespace Server.Items
{
    ...
    {
        [Constructable]
        public WorkingSpots() : base(0x6519)
        {
            Name = "working spot";
            Visible = false;
            Movable = false;
        ...
        public static void Cleanup(Item spot)
        {
            ...
            {
                if (!(item is WorkingSpots) && item.X == spot.X && item.Y == spot.Y && item.Map == spot.Map)
                {
                    clean.Add(item);
                }
            }
        ...
        public static void Setup()
        {
            ...
            {
                if (spot is WorkingSpots && (Utility.RandomMinMax(1, 4) > 1 || Server.Misc.Worlds.GetRegionName(spot.Map, spot.Location) == "the Basement" || Server.Misc.Worlds.GetRegionName(spot.Map, spot.Location) == "the Dojo"))
                    spots.Add(spot);
            }
            for (int i = 0; i < spots.Count; ++i)
            {
                ...
                else if (spot.Name == "butcher") { Mobile citizen = new TradesmanButcher(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 1); }
                else if (spot.Name == "bard") { Mobile citizen = new TradesmanBard(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 1); }
                else if (spot.Name == "shrine") { Mobile citizen = new TrainingSpirits(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 6); }
            }
        }
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                if (hasCitizen)
                {
                    ...
                    else if (spot.Name == "potion" && citizen is TradesmanAlchemist)
                    {
                        ...
                        if (item.ItemID >= 0x5760 && item.ItemID <= 0x5769) { item.Z = item.Z - 2; }
                    }
                    ...
                    else if (spot.Name == "carcass" && citizen is TradesmanButcher) { Item item = new ButcherHit(); item.MoveToWorld(spot.Location, spot.Map); item.Weight = -2.0; }
                    else if (spot.Name == "music" && citizen is TradesmanBard) { Item item = new BardHit(); item.MoveToWorld(spot.Location, spot.Map); item.Weight = -2.0; }
                }
            }
        }
