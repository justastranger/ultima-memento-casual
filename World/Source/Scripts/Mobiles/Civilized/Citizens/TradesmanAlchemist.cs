...
namespace Server.Items
{
    public class CauldronHit : Item
    {
        ...
        public CauldronHit() : base(0x227D)
        {
            Name = "cauldron";
            Movable = false;
            ItemID = Utility.RandomList(0x227D, 0x5711, 0x5761, 0x5766);
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from is Citizens)
            {
                ...
                if (ItemID >= 0x5710 && ItemID <= 0x5717) { Hue = Utility.RandomColor(0); }
                else if (ItemID >= 0x5760 && ItemID <= 0x5769) { Hue = Utility.RandomColor(0); }
                else { ItemID = Utility.RandomList(0x227D, 0x2284, 0x228B, 0x2292); }
    ...
    public class CrateOfReagents : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                else if (ItemID == 0x6415) { from.AddToBackpack(new BitterRoot(CrateQty)); }
                else if (ItemID == 0x6416) { from.AddToBackpack(new BlackSand(CrateQty)); }
                else if (ItemID == 0x6417) { from.AddToBackpack(new BloodRose(CrateQty)); }
                else if (ItemID == 0x6418) { from.AddToBackpack(new DriedToad(CrateQty)); }
                else if (ItemID == 0x6419) { from.AddToBackpack(new Maggot(CrateQty)); }
                else if (ItemID == 0x641A) { from.AddToBackpack(new MummyWrap(CrateQty)); }
                else if (ItemID == 0x641B) { from.AddToBackpack(new VioletFungus(CrateQty)); }
