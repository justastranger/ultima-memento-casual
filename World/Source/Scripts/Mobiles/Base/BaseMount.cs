...
namespace Server.Mobiles
{
    public abstract class BaseMount : BaseCreature, IMount
    {
        ...
        {
            if (Rider != null) { Server.Mobiles.EtherealMount.EthyDismount(Rider); }
            Rider = null;
        ...
        public static void Ride(BaseMount mount, Mobile rider)
        {
            ...
            else if (mount is TigerRiding) { mount.Hue = 0x54F; }
            else if (mount is SabretoothTigerRiding) { mount.Hue = 0x54F; }
        ...
        {
            if (this is Wyverns || this is TigerRiding || this is SabretoothTigerRiding)
                this.Hue = 0;
    ...
    public class MountItem : Item, IMountItem
    {
        ...
        public override DeathMoveResult OnParentDeath(Mobile parent)
        {
            if (m_Mount != null)
            {
                if (m_Mount.Rider != null) { Server.Mobiles.EtherealMount.EthyDismount(m_Mount.Rider); }
                m_Mount.Rider = null;
            }
