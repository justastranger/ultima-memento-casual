    ...
    public class LureStoneSpell : HerbalistSpell
    {
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                Item item = new InternalItema(loc, Caster.Map, Caster, (double)(Server.Items.BasePotion.EnhancePotions(Caster)));
                stonex = p.X;
                stoney = p.Y - 1;
                stonez = p.Z;
                Point3D loca = new Point3D(stonex, stoney, stonez);
                Item itema = new InternalItemb(loca, Caster.Map, Caster, (double)(Server.Items.BasePotion.EnhancePotions(Caster)));
            }
            FinishSequence();
        }
        ...
        private class InternalItemb : Item
        {
            ...
            public InternalItemb(Point3D loc, Map map, Mobile caster, double enhance) : base(0x1356)
            {
                Visible = false;
                Movable = false;
