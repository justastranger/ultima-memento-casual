    ...
    public class GraspingRootsSpell : HerbalistSpell
    {
        ...
        public void Target(Mobile m)
        {
            ...
            else if (CheckHSequence(m))
            {
                ...
                // Resist if Str + Dex / 2 is greater than DamageSkill eg. Veterinary seems to work??
                if ((Caster.Skills[DamageSkill].Value) < ((m.Str + m.Dex) * 0.5))
                    duration *= 0.5;
                ...
                BuffInfo.AddBuff(m, new BuffInfo(BuffIcon.GraspingRoots, 1063548, TimeSpan.FromSeconds(duration), m));
