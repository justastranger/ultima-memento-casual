...
namespace Server.Spells.Herbalist
{
    public class ProtectiveFairySpell : HerbalistSpell
    {
        ...
        public override bool CheckCast()
        {
            ...
            {
                Caster.PrivateOverheadMessage(MessageType.Regular, 0x14C, false, "You lack the understanding to even command a fairy.", Caster.NetState);
                return false;
            }
        ...
        public override void OnCast()
        {
            if (CheckSequence())
            {
                TimeSpan duration = TimeSpan.FromSeconds((Caster.Skills[CastSkill].Value + Caster.Skills[DamageSkill].Value) * 9);
