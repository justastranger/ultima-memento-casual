    ...
    public class MagicLockSpell : MagerySpell
    {
        ...
        public void Target(object o)
        {
                ...
                if (targ is LockableContainer)
                {
                    ...
                    else if (CheckSequence())
                    {
                        ...
                        Effects.SendLocationParticles(
                            ...
                            0x376A, 9, 32, PlayerSettings.GetMySpellHue(true, Caster, 0), 0, 5020, 0);
                        ...
                        box.LockLevel = (int)(Spell.ItemSkillValue(Caster, SkillName.Magery, false));
            ...
            else if (o is BaseCreature)
            {
                ...
                {
                    int level = IntelligentAction.GetCreatureLevel((Mobile)o) + 10;
                    int magery = (int)(Spell.ItemSkillValue(Caster, SkillName.Magery, false));
        ...
        private class InternalTimer : Timer
        {
            ...
            {
                double val = Spell.ItemSkillValue(caster, SkillName.Magery, false) / 2.0;
                if (val < 10)
                    val = 10;
                else if (val > 60)
                    val = 60;
...
namespace Server.Mobiles
{
    ...
    public class LockedCreature : BaseCreature
    {
        ...
        public LockedCreature(int job, int skills, int time, int maxhits, int maxstam, int maxmana, int str, int dex, int iq, int poison, int immune, int anger, int idle, int death, int attack, int hurt) : base(AIType.AI_Melee, FightMode.Closest, 10, 1, 0.3, 0.6)
        {
            ...
            if (job == 1)
            {
                SetSkill(SkillName.Psychology, (double)skills);
                ...
                SetSkill(SkillName.FistFighting, (double)skills);
            }
            else
            {
