        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (CheckSequence())
            {
                ...
                for (int i = 0; i < targets.Count; ++i)
                {
                    ...
                    double dispelChance = (50.0 + ((100 * (Spell.ItemSkillValue(Caster, SkillName.Magery, false) - bc.DispelDifficulty)) / (bc.DispelFocus * 2))) / 100;

                    if (dispelChance > Utility.RandomDouble())
                    {
                        Effects.SendLocationParticles(EffectItem.Create(m.Location, m.Map, EffectItem.DefaultDuration), 0x3728, 8, 20, PlayerSettings.GetMySpellHue(true, Caster, 0), 0, 5042, 0);
                    ...
                    {
                        if (Spell.ItemSkillValue(Caster, SkillName.Magery, false) > Utility.RandomMinMax(1, 100))
                        {
                            Effects.SendLocationParticles(EffectItem.Create(m.Location, m.Map, EffectItem.DefaultDuration), 0x3728, 8, 20, PlayerSettings.GetMySpellHue(true, Caster, 0), 0, 5042, 0);
                            Effects.PlaySound(m, m.Map, 0x201);
                            m.Delete();
                    ...
                    else
                    {
                        ...
                        m.FixedEffect(0x3779, 10, 20, PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                }
            }
