    ...
    public class ParalyzeFieldSpell : MagerySpell
    {
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                TimeSpan duration = TimeSpan.FromSeconds(3.0 + (Spell.ItemSkillValue(Caster, SkillName.Magery, false) / 3.0));
                ...
                for (int i = -2; i <= 2; ++i)
                {
                    ...
                    Effects.SendLocationParticles(EffectItem.Create(loc, Caster.Map, EffectItem.DefaultDuration), 0x376A, 9, 10, PlayerSettings.GetMySpellHue(true, Caster, 0), 0, 5048, 0);
                }
            }
        ...
        public class InternalItem : Item
        {
            ...
            public InternalItem(Mobile caster, int itemID, Point3D loc, Map map, TimeSpan duration) : base(itemID)
            {
                ...
                Hue = PlayerSettings.GetMySpellHue(false, caster, 0);
            ...
            public override bool OnMoveOver(Mobile m)
            {
                if (Visible && !Server.Spells.SpellHelper.isFriend(m_Caster, m))
                {
                    ...
                    duration = 2.0 + ((int)(Spell.ItemSkillValue(m_Caster, SkillName.Psychology, false) / 10) - (int)(m.Skills[SkillName.MagicResist].Value / 10)) + nBenefit;
                    ...
                    BuffInfo.AddBuff(m, new BuffInfo(BuffIcon.ParalyzeField, 1063621, TimeSpan.FromSeconds(duration), m));

                    m.PlaySound(0x204);
                    m.FixedEffect(0x376A, 10, 16, PlayerSettings.GetMySpellHue(true, m_Caster, 0), 0);
