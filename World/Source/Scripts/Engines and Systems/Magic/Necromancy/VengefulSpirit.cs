    ...
    public class VengefulSpiritSpell : NecromancerSpell
    {
        ...
        public void Target(Mobile m)
        {
            ...
            else if (CheckHSequence(m))
            {
				 ...
				 * A Revenant's strength is determined by the Necromancy and Spiritualism skills of the Caster.
				 * The effect lasts for ((Spiritualism skill level * 80) / 120) + 10 seconds.
				 */

                TimeSpan duration = TimeSpan.FromSeconds(((GetDamageSkill(Caster) * 80) / 120) + 10 + nBenefit);
