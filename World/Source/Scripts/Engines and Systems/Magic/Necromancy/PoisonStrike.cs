    ...
    public class PoisonStrikeSpell : NecromancerSpell
    {
        ...
        public void Target(Mobile m)
        {
            if (CheckHSequence(m))
            {
                ...
                double sdiBonusPVM = (double)AosAttributes.GetValue(Caster, AosAttribute.SpellDamage) / 100;
                if (MyServerSettings.SpellDamageIncreaseVsMonsters() > 0 && sdiBonusPVM > MyServerSettings.SpellDamageIncreaseVsMonsters())
                {
                    sdiBonusPVM = MyServerSettings.SpellDamageIncreaseVsMonsters();
                }

                double sdiBonusPVP = (double)AosAttributes.GetValue(Caster, AosAttribute.SpellDamage) / 100;
                if (MyServerSettings.SpellDamageIncreaseVsPlayers() > 0 && sdiBonusPVP > MyServerSettings.SpellDamageIncreaseVsPlayers())
                {
                    sdiBonusPVP = MyServerSettings.SpellDamageIncreaseVsPlayers();
                }
