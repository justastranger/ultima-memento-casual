...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        public void Kick(Mobile from, Mobile targ)
        {
            ...
            else if (targ is HouseVisitor)
            {
                from.SendMessage("You cannot eject a visitor.");
            }
        ...
        {
            int MaxHouses = MyServerSettings.HousesPerAccount();
            int HasHouses = 0;
        ...
        public bool IsOwner(Mobile m)
        {
            ...
            if (m == m_Owner || m.AccessLevel >= AccessLevel.GameMaster)
                return true;
            else if (m_CoOwners.Contains(m) && MySettings.S_HouseOwners)
                return true;
