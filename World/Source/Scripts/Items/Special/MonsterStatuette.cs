...
namespace Server.Items
{
	...
	public class MonsterStatuette : Item
	{
		...
		public MonsterStatuette( MonsterStatuetteType type ) : base( MonsterStatuetteInfo.GetInfo( type ).ItemID )
		{
			...
			Name = CliLocTable.Lookup( MonsterStatuetteInfo.GetInfo( m_Type ).LabelNumber );
		}
		...
		{
			if ( m_TurnedOn && IsLockedDown && (!m.Hidden || m.AccessLevel <= AccessLevel.Counselor) && Utility.InRange( m.Location, this.Location, 2 ) && !Utility.InRange( oldLocation, this.Location, 2 ) )
			{
