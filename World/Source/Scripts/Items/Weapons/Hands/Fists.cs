...
namespace Server.Items
{
    public class Fists : BaseMeleeWeapon
    {
        ...
        {
            double wresValue = defender.Skills[SkillName.FistFighting].Value;
            double anatValue = defender.Skills[SkillName.Anatomy].Value;
            double evalValue = defender.Skills[SkillName.Psychology].Value;
        ...
        public override TimeSpan OnSwing(Mobile attacker, Mobile defender)
        {
            if (attacker.StunReady)
            {
                ...
                {
                    if (attacker.Skills[SkillName.Anatomy].Value >= 80.0 && attacker.Skills[SkillName.FistFighting].Value >= 80.0)
                    {
                        if (attacker.Stam >= 15)
                        {
                            attacker.Stam -= 15;
            ...
            else if (attacker.DisarmReady)
            {
                if (attacker.CanBeginAction(typeof(Fists)))
                {
                    ...
                    {
                        if (attacker.Skills[SkillName.ArmsLore].Value >= 80.0 && attacker.Skills[SkillName.FistFighting].Value >= 80.0)
                        {
                            if (attacker.Stam >= 15)
                            {
