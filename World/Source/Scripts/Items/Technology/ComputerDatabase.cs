    ...
    public class ComputerDatabase : Item
    {
        ...
        public class ComputerDatabaseGump : Gump
        {
            ...
            public override void OnResponse(NetState state, RelayInfo info)
            {
                ...
                if (info.ButtonID == 1) { skinColor = Utility.RandomSkinColor(); }
                ...
                else if (info.ButtonID == 9) { hairColor = Utility.RandomHairColor(); }
                ...
                {
                    from.Hue = skinColor;
                    from.RecordSkinColor = skinColor;
                    from.SendGump(new ComputerDatabaseGump(from));
                    from.SendSound(0x54B);
                    from.RecordFeatures(true);
                }
                else if (hairColor > 0)
                {
                    from.HairHue = hairColor;
                    from.FacialHairHue = hairColor;
                    from.RecordHairColor = hairColor;
                    from.RecordBeardColor = hairColor;
                    from.SendGump(new ComputerDatabaseGump(from));
                    from.SendSound(0x54B);
                    from.RecordFeatures(true);
                }
