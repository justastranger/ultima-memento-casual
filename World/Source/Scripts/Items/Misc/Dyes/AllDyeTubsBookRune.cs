...
{
    public class AllDyeTubsBookRune : DyeTub
    {
        ...
        [Constructable]
        public AllDyeTubsBookRune()
        {
            Name = "Rune Book Dye Tub";
            Weight = 5.0;
            Hue = TheHue;
            DyedHue = TheHue;
            Charges = 100;
        ...
        public void DoOut(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                ...
                from.Target = new AllDyeTubsBookRuneTarget(this);
            }
            else
            {
        ...
        public AllDyeTubsBookRune(Serial serial) : base(serial) { }
        ...
        public class AllDyeTubsBookRuneTarget : Target
        {
            private AllDyeTubsBookRune m_Tub;

            public AllDyeTubsBookRuneTarget(AllDyeTubsBookRune dyetub) : base(12, false, TargetFlags.None)
            {
                m_Tub = dyetub;
            }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Item)
                {
                    ...
                    if ((item is Runebook) && (from.Backpack != null && from.Backpack.ConsumeTotal(typeof(Gold), 100)))
                    {
