...
namespace Server.Items
{
    public class MongbatDartBoard : AddonComponent
    {
        ...
        public void Throw(Mobile from)
        {
            ...
            if (nAnimate == 1 && ItemID >= 0x1953 && ItemID <= 0x1955) { ItemID = 0x1954; }
            else if (nAnimate == 1 && ItemID >= 0x1950 && ItemID <= 0x1952) { ItemID = 0x1951; }

            if (nAnimate == 1)
            {
                Timer.DelayCall(TimeSpan.FromSeconds(0.7), new TimerCallback(OnMongbatReset));
                Timer.DelayCall(TimeSpan.FromSeconds(0.2), new TimerCallback(OnMongbatHit));
            }
            else if (nHit == 1)
            {
                Timer.DelayCall(TimeSpan.FromSeconds(0.2), new TimerCallback(OnMongbatNick));
            }
        ...
        {
            if (ItemID != 0x1951 && ItemID != 0x1954) { Timer.DelayCall(TimeSpan.FromSeconds(0.1), new TimerCallback(OnMongbatReset)); }
            else if (ItemID >= 0x1953 && ItemID <= 0x1955) { ItemID = 0x1953; }
            else { ItemID = 0x1950; }
        }

        public virtual void OnMongbatHit()
        {
            Effects.PlaySound(Location, Map, 0x149);
        }

        public virtual void OnMongbatNick()
        {
