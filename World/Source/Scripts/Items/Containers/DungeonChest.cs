...
namespace Server.Items
{
    public class DungeonChest : LockableContainer
    {
        ...
        public static void SetupChest(Item box)
        {
            ...
            else if (reg.IsPartOf("Dungeon Rock"))
            {
                design = 16; // METAL CHESTS, STONE CHESTS, URNS
            }
            ...
            else if (box.ItemID == 0x5718) { chest.ContainerFlip = 0x5719; }
            else if (box.ItemID == 0x571A) { chest.ContainerFlip = 0x571B; }
            else if (box.ItemID == 0x5752) { chest.ContainerFlip = 0x5753; }
            else if (box.ItemID == 0x5719) { chest.ContainerFlip = 0x5718; }
            else if (box.ItemID == 0x571B) { chest.ContainerFlip = 0x571A; }
            else if (box.ItemID == 0x5753) { chest.ContainerFlip = 0x5752; }
        ...
        public static void OpenCoffin(Mobile from, int item, int level)
        {
            if (Utility.RandomMinMax(1, 10) == 1) // 10% CHANCE FOR RAISING DEAD IN COFFINS
            {
                ...
                {
                    int seance = (int)(from.Skills[SkillName.Spiritualism].Value);
