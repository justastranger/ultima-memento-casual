...
{
    public class HiddenTrapDoor : LockableContainer
    {
        [Constructable]
        public HiddenTrapDoor(int level) : base(0x02C4)
        {
            Name = "trap door";
            ItemID = Utility.RandomList(0x02C1, 0x02C2, 0x02C3, 0x02C4);
            Movable = false;
            int LockWatch = ContainerFunctions.LockTheContainer(level, this, 0);
            new InternalTimer(this).Start();
            LiftOverride = true;
        }

        public HiddenTrapDoor(Serial serial) : base(serial)
        {
            new InternalTimer(this).Start();
        }
        ...
        private class InternalTimer : Timer
        {
            ...
            public InternalTimer(Item box) : base(TimeSpan.FromMinutes(15.0))
            {
    ...
    public class HiddenBox : HiddenContainer
    {
        ...
        public HiddenBox(int level, string my_location, Mobile finder) : base()
        {
            ...
            else if (this.ItemID == 0x5718) { ContainerFlip = 0x5719; }
            else if (this.ItemID == 0x571A) { ContainerFlip = 0x571B; }
            else if (this.ItemID == 0x5752) { ContainerFlip = 0x5753; }
            else if (this.ItemID == 0x5719) { ContainerFlip = 0x5718; }
            else if (this.ItemID == 0x571B) { ContainerFlip = 0x571A; }
            else if (this.ItemID == 0x5753) { ContainerFlip = 0x5752; }
