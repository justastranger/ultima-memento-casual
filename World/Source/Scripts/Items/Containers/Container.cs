...
namespace Server.Items
{
    public abstract class BaseContainer : Container
    {
        ...
        private bool m_SearchedThis;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool SearchedThis
        {
            get { return m_SearchedThis; }
            set { m_SearchedThis = value; }
        }
        ...
        public static bool PutStuffInContainer(Mobile from, int box, Item item)
        {
            ...
            if (box == 1 && from.ContainerLoot > 0)
                bag = PutStuffBox(from, from.ContainerLoot);
            else if (box == 2 && from.ContainerCraft > 0)
                bag = PutStuffBox(from, from.ContainerCraft);
        ...
        public static Container PutStuffBox(Mobile from, int serialnum)
        {
            ...
            {
                if (i is Container && i.Serial == serialnum)
                    return (Container)i;
            }
            return null;
        }

        public static void ContainerSetTarget(Mobile from, int box)
        {
        ...
        private class ContainSetTarg : Target
        {
            ...
            public ContainSetTarg(Mobile from, int box) : base(2, false, TargetFlags.None)
            {
                m_From = from;
                m_Box = box;
            }
            ...
            {
                if (targeted is Item && targeted is Container)
                {
                    Container box = (Container)targeted;
    ...
    public class SmallHollowBook : BaseContainer
    {
        ...
        public void TitleBook()
        {
            if (ColorText1 == null && X > 0)
            {
                ...
                ColorText3 = "By " + Server.Misc.RandomThings.GetRandomAuthor();
                ColorHue3 = "5DE18D";
            }
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (version < 1)
            {
                ColorText3 = "By " + reader.ReadString();
                ColorHue3 = "5DE18D";
            }
        }
