...
namespace Server.Items
{
    ...
    public class HiLoCards : Item
    {
        ...
        private int[] m_HueTable = new int[] { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
        ...
        private int m_CreditATMIncrements = 2000;
        ...
        public bool Active
        {
            ...
            set
            {
                if (!m_Active && value)
                {
                    ...
                    this.PublicOverheadMessage(0, (this.Hue == 907 ? 0 : this.Hue), false, "Game online!");
                }
                else if (m_Active && !value)
                {
                    m_OrigHue = this.Hue;
                    this.Hue = 1001;
                    this.PublicOverheadMessage(0, this.Hue, false, "Game offline.");
                }
                m_Active = value;
        ...
        public HiLoCards(Serial serial) : base(serial)
        {
        }
            ...
            if (m_InUseBy != null)
            {
                if (!m_InUseBy.InRange(GetWorldLocation(), 3) || m_InUseBy.Map == Map.Internal)
                {
                    ...
                    if (m_OnCredit != 0)
                    {
                        ...
                        m_InUseBy.SendMessage("Hey, you left some cash at the table! Cashing out.");
                        DoCashOut(m_InUseBy); // Give them their winnings
                    }
                    else
                        m_InUseBy.SendMessage("You walk away from this game.");
                    InUseBy = null;
                    InvalidateProperties();
                }
            }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (!m_Active)
            {
                ...
                from.SendMessage("Sorry, this game is currently down for maintenance.");
                return;
            }
            ...
            {
                string text = String.Format("{0} is currently playing this game.", m_InUseBy.Name);
        ...
        public void PlayHiLo(Mobile from, int cost)
        {
            ...
            {
                from.SendMessage("You are too far away tp play.");
                RemovePlayer(from);
                return;
            }
            if (m_TestMode)
        ...
        public bool CashCheck(Mobile m, out int amount)
        {
            ...
            for (int i = 0; i < packlist.Count; ++i)
            {
                ...
                if (item != null && !item.Deleted && item is BankCheck)
                {
                    ...
                    {
                        m.SendMessage("There's a problem trying to cash a check in your backpack, this game is offline.");
                        HiLoOffline(9503);
                        return false;
                    }
                    return true;
        ...
        public void DoCashOut(Mobile from)
        {
            ...
            if (m_OnCredit < 0) // This should never happen but protect against some kind of overflow and a wild payout
            {
                ...
                from.SendMessage("There's a problem with this game's gold amount, this game is offline.");
                HiLoOffline(9502);
                return;
            }
            if (m_OnCredit < 1000)
            {
                ...
                {
                    from.SendMessage("There's a problem returning your gold, this game is offline.");
                    HiLoOffline(9500);
                    return;
                }
            }
...
namespace Server.Gumps
{
    public class HiloCardGump : Gump
    {
        ...
        public HiloCardGump(Mobile player, HiLoCards hilocards, int backdesign, int[] card, int progress, bool playing, string message) : base(20, 20)
        {
            Closable = true;
            Disposable = true;
            Dragable = true;
            ...
            AddLabel(m_xSize / 2 - 50, 15, m_HiLoCards.Hue, "High-Low Card Game");
            for (int x = 0; x < 3; x++)
            {
                if (card[x] == -1)
        ...
        private void DisplayHelpGump()
        {
            ...
            string text = Color("Objective: ", HC, false) +
                ...
                Color(" Do not sit idle at the game (over 5 mins) or someone else can take it away from you.", TC, false) +
                ...
                "</p><p>" + Color(Center("High-Low Card Game"), 0x696969, false) +
                Color("This is a simple card game you can enjoy while sitting in a tavern.", CR, false) +
                "</p></BASEFONT>";
        ...
        private void CEOCookie(int hue, Mobile m)
        {
            ...
            AddLabel(15, 110, hue, "Enjoying the High-Low Card Game?\" :)");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            else
            {
                ...
                else if (info.ButtonID == m_Base + 302) // ATM
                {
                    ...
                    {
                        message = "This game is at or over its credit limit.";
                    }
                    else if (m_HiLoCards.TestMode)
                    {
                        message = "Game in test mode, doubleclick for credits.";
                    }
                    else
                    {
