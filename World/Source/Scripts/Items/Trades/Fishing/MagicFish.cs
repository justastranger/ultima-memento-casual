    ...
    public abstract class BaseMagicFish : Item
    {
        ...
        public virtual bool Apply(Mobile from)
        {
            ...
            if (Type == StatType.Str)
                BuffInfo.AddBuff(from, new BuffInfo(BuffIcon.FishStr, 1063672, TimeSpan.FromMinutes(1.0), from, Bonus.ToString()));
            if (Type == StatType.Dex)
                BuffInfo.AddBuff(from, new BuffInfo(BuffIcon.FishDex, 1063674, TimeSpan.FromMinutes(1.0), from, Bonus.ToString()));
            if (Type == StatType.Int)
                BuffInfo.AddBuff(from, new BuffInfo(BuffIcon.FishInt, 1063676, TimeSpan.FromMinutes(1.0), from, Bonus.ToString()));
