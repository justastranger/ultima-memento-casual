...
namespace Arya.Misc
{
    public class MultiGenerator
    {
        ...
        {
            CommandSystem.Register("MultiGen", AccessLevel.Administrator, new CommandEventHandler(OnMultiGen));
        }
        ...
        private static void PickerCallback(Mobile from, Map map, Point3D start, Point3D end, object state)
        {
            ...
            if (m_CustomOutputDirectory != null)
                path = Path.Combine(m_CustomOutputDirectory, string.Format(@"TheMulti\{0}Multi.txt", name));
            else
                path = Path.Combine(Core.BaseDirectory, string.Format(@"TheMulti\{0}Multi.txt", name));
        ...
        private static void AddIdentifyAddOnComponent(MultiIdentifyAddon ai, int item, int xoffset, int yoffset, int zoffset, int hue, int lightsource, string name, int amount)
        {
            if (ai == null)
                return;
            AddonComponent ac;
