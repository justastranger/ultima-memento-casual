...
namespace Server.Commands
{
    public class CommandHandlers
    {
        public static void Initialize()
        {
            ...
            Register("Inn", AccessLevel.GameMaster, new CommandEventHandler(Inn_OnCommand));
                ...
                if (pets.Count > 0)
                {
                    ...
                    for (int i = 0; i < pets.Count; ++i)
                    {
                        ...
                        if (pet is IMount)
                        {
                            ...
                            {
                                Server.Mobiles.EtherealMount.EthyDismount(((IMount)pet).Rider);
                            }
        ...
        private class InnTarget : Target
        {
            public InnTarget() : base(-1, false, TargetFlags.None)
            {
            }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Mobile)
                {
                    ...
                    InnRoom inn = (m.Player ? m.InnRoom : m.FindInnNoCreate());

                    if (inn != null)
                    {
                        CommandLogging.WriteLine(from, "{0} {1} opening inn room of {2}", from.AccessLevel, CommandLogging.Format(from), CommandLogging.Format(targeted));
                    ...
                    {
                        from.SendMessage("They have no inn room.");
                    }
                }
            }
        }
        ...
        [Usage("Inn")]
        [Description("Opens the inn room of a given target.")]
        public static void Inn_OnCommand(CommandEventArgs e)
        {
            e.Mobile.Target = new InnTarget();
        }
                    ...
                    for (int i = 0; i < targ.Items.Count; ++i)
                    {
                        ...
                        if (item is IMountItem)
                        {
                            ...
                            if (mount != null)
                            {
                                if (mount.Rider != null)
                                {
                                    Server.Mobiles.EtherealMount.EthyDismount(mount.Rider);
                                }
                                mount.Rider = null;
                            }
