...
namespace Server.Items
{
	...
	public class Container : Item
	{
		...
		public int GumpID
		{
			get
			{
				...
				if ( this is BankBox ) 
					...
					return ( m_GumpID == -1 ? DefaultGumpID : m_GumpID ); }
			set
			{
				if ( this is InnRoom ) 
					m_GumpID = 0xB3B;
		...
		public virtual bool CheckContentDisplay( Mobile from )
		{
			...
			if ( root == null || root is Item || root == from || from.AccessLevel > AccessLevel.Counselor )
				return true;
						...
						if ( split.Length >= 3 )
						{
							...
							if ( split.Length >= 4 )
							{
								...
								for ( int i = 0; i < aIDs.Length; i++ )
								{
									...
									{
										Console.WriteLine( @"Warning: double ItemID entry in containers.cfg" );
									}
									else
									{
										m_Table[id] = data;
